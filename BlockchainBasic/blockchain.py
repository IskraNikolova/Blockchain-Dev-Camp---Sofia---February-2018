...
class Blockchain(object):
    ...
    def resolve_conflicts(self):
        ...
        for node in neighbours:
            ...
            if response.status_code == 200:
                ...
                # Check if the length is longer and the chains is valid
                if lenght > max_length and self.valid_chain(chain):
                    max_length = lenght
                    new_chain = chain
        # Repalce our chain if we've discovered a new, valid chain, longer than ours
        if new_chain:
    ...
    def hash(block):
        """
        Creates a SHA-256 hash af a Block
...
def mine():
    ...
    # We must receive a revard for finding the proof.
